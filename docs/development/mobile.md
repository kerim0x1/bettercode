...
- `demo-terminal.yaml`: a terminal from a chat's menu, a command typed into it and the demo shell's answer. Likewise the only check of the terminal's page (xterm.js) on a device. It reads the terminal's text from its rows, which the page leaves readable to accessibility services while no screen reader runs (with VoiceOver or TalkBack on, xterm's own screen reader mode takes over).
